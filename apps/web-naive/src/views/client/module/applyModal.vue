...
const title = computed(() => (props.mode === 'create' ? '新增客户端模块' : '编辑客户端模块'));
  ...
  <NModal v-model:show="show" preset="card" :title="title" class="w-[520px]">
    ...
    >
      <NFormItem label="模块名称" path="name">
        <NInput v-model:value="formModel.name" placeholder="例如：客户管理" />
      </NFormItem>
      <NFormItem label="模块编码" path="code">
        <NInput v-model:value="formModel.code" placeholder="例如：client" />
      </NFormItem>
