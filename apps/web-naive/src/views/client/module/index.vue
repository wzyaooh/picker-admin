...
import {
  createClientModuleApi,
  deleteClientModuleApi,
  getClientModuleListApi,
  updateClientModuleApi,
} from '#/api';

import ApplyModal from './applyModal.vue';

defineOptions({ name: 'ClientModulePage' });
...
async function fetchModules() {
  ...
  try {
    ...
    dataSource.value = (result.pageData || []).map((item: any) => ({
      id: item.id,
      name: item.name,
      code: item.code,
...
function handleDelete(row: ModuleRecord) {
  dialog.warning({
    ...
    onPositiveClick: async () => {
      try {
        await deleteClientModuleApi(row.id);
    ...
    onPositiveClick: async () => {
      try {
        ...
        for (const id of keys) {
          await deleteClientModuleApi(id);
        }
        message.success('删除成功');
        await fetchModules();
...
async function handleModalSubmit(values: {
  ...
  try {
    if (modalMode.value === 'create') {
      ...
      if (!id) return;
      await updateClientModuleApi(id, {
        name: values.name,
        code: values.code,
        enable: values.enable,
        description: values.description,
...
<template>
  <div class="p-4">
    <NCard title="客户端模块管理" :bordered="false" size="small">
