...
import { getButtonPermissionsApi, createPermissionApi, updatePermissionApi, deletePermissionApi, type PermissionApi } from '#/api';
...
const buttons = ref<PermissionApi.Permission[]>([]);
...
async function loadButtons() {
  ...
  loading.value = true;
  try {
    const result = await getButtonPermissionsApi(props.menuId);
...
// 开始编辑
function handleEdit(row: PermissionApi.Permission) {
  showAddModal.value = false;
...
// 保存编辑
async function handleSaveEdit(row: PermissionApi.Permission) {
  ...
  try {
    await updatePermissionApi(row.id, {
...
// 删除按钮
async function handleDelete(row: PermissionApi.Permission) {
  try {
...
// 表格列定义
const columns: DataTableColumns<PermissionApi.Permission> = [
  {
    title: '名称',
    key: 'name',
    width: 180,
        ...
        >
          ...
          <NFormItem label="编码" path="code" class="mt-2">
            <NInput
              ...
              placeholder="请输入按钮编码，如：user:create"
              clearable
              size="large"
              @keyup.enter="handleAddButton"
            />
