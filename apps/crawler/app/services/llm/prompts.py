...
所有 Anthropic Claude 调用使用的 prompt 模板集中管理。
包含：结构化分析、文章生成、教程生成、文章润色四轮 prompt。
"""

# ==================== Prompt 模板 ====================

# 通用数据头部（所有类型共享）
DATA_HEADER = """你是一位在技术圈拥有 100 万粉丝的博主，风格像少数派 + 阮一峰 + 陈皓（酷壳）的结合体。
...
ARTICLE_PROMPT = """你是一位在技术圈拥有 100 万粉丝的博主，同时也是有 15 年经验的全栈架构师。
...
TUTORIAL_PROMPT = """你是一位在技术圈拥有 100 万粉丝的博主，同时也是有 15 年经验的全栈架构师。
...
POLISH_HEADER = """你是原文的高级编辑，你的任务是对一篇已有的技术文章进行**实质性润色**，让它更好。
...
POLISH_ANALYSIS_PROMPT = POLISH_HEADER + """
## 润色指令 — 深度解析文章（严格遵守）
...
POLISH_TUTORIAL_PROMPT = POLISH_HEADER + """
## 润色指令 — 实践教程文章（严格遵守）
