...
export class CreateScheduledTaskTables20260216100000
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    // 创建 scheduled_task 表
    const taskTable = await queryRunner.getTable('scheduled_task');
    if (!taskTable) {
    ...
    const logTable = await queryRunner.getTable('scheduled_task_log');
    if (!logTable) {
      await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    const logTable = await queryRunner.getTable('scheduled_task_log');
    if (logTable) {
      await queryRunner.query(`DROP TABLE \`scheduled_task_log\``);
    }
